...
def get_coefficients(V, im, mean_im):
    coefs = [np.dot(V[i,:], (im-mean_im)) for i in range(V.shape[0])]
    return coefs # return the coefficients 


# This function gets reconstruction of image, im 
# from the principal components and the mean images 
...
for i in range(valIm.shape[0]):
    valIm[i,:] = valIm[i,:]/255.0

for i in range(testIm.shape[0]):
    testIm[i,:] = testIm[i,:]/255.0
...
# Ignore the first face as that is the average face given by the algorithm. 
eigenFaces = eigenFace[1:]
# Save and Display the top 25 principal components 
displaySavComps(eigenFaces, train1Shape, 25) 

maxAccuracy = 0.0 # initialize accuracy to 0 percent 
maxK = 0 # initialize maxK value to be 2 
for k in [2, 5, 10, 20, 50, 80, 100, 150, 200]:
#for k in [50]: # 
    coeffTrains = [None] * trainIm.shape[0]
    #print coeffTrains
    m = 0 
    for eachTrainIm in trainIm:
        coeffTrain = get_coefficients(eigenFaces[:k], eachTrainIm, averageFace)
        coeffTrains[m] = coeffTrain 
        m += 1
    #print coeffTrains
    numCorrect = 0.0 # initialize number of faces correctly predicted 
    numWrong = 0.0 # initialize number of faces wrongly predicted     
    
    j = 0
    
    # Note: Code only works with eactly 800,80,80 combination! 
    # For each validation image 
    for eachValIm in valIm: 
        # Get the coefficients of reconstruction for this validation image 
        coeffVal = get_coefficients(eigenFaces[:k], eachValIm, averageFace)
        currMin = sys.float_info.max # initialize to largest possible flaot 
        indexTrain = 0 # initialize index to first training image 
        
        # Compare with every training image 
        i = 0 
        for eachTrainIm in trainIm:
            #imsave("./reconstructionPngTrainingSet/" + str(k)+ "-" + str(i) +'pc.png', get_reconstruction(eigenFace[:k], eachTrainIm, averageFace).reshape(im_shape))
            # Can speed up here by only calculating each of these ones and indexing into it 
            # calculate the Sum of Square Difference between the coefficients 
            # from the validationSet to the training set 
            # Calculate the SSD
            # get the coefficient from train as calculated 
            differenceSSD = np.array(coeffVal) - coeffTrains[i] 
            differenceSSD = differenceSSD * differenceSSD 
            sumSSD = differenceSSD.sum() 
            if sumSSD < currMin:
                currMin = sumSSD
                indexTrain = i # set the best possible image to be this image 
                #print 'currMin: ' + str(currMin) + ' indexTrain: ' + str(indexTrain) 
            i += 1 #increment the index for training image 
        # Now, already have the best possible match from trainingSet 
        # Check to see if it is the correct image 
        #print indexTrain
        #print j
        #print indexTrain/100
        #print j/10
        # Initialize both to boys 
        A = 1
        B = 1
        if(indexTrain/100 < 3):
            A = 1
        else:
            A = 2 # trainingIndex is a girl              
            
            
        if(j/10 < 3):
            B = 1
        else:
            B = 2 # testIndex is a girl           
        
        # If predicted correct gender 
        if (A == B):
            numCorrect += 1.0
        else:            
            numWrong += 1.0
            #print 'Wrong: indexTrain: ' + str(indexTrain) + " indexVal: " + str(j) 
        j += 1 # increment the index for validationImage 
    # Get the accuracy for this value of k
    #print numCorrect
    #print numWrong
    currAccuracy =  (numCorrect/(numCorrect + numWrong))* 100
    print 'K:' + str(k) + ' numCorrect: ' + str(numCorrect) + ' numWrong: ' + str(numWrong) + " accuracy: " + str(currAccuracy)
    if (currAccuracy > maxAccuracy): 
        maxAccuracy = currAccuracy
        maxK = k 

# Output value of K that is the most accurate    
print 'Maximum K is : ' + str(maxK) 
        
# Now with the value of K that is most accurate, 
# Repeat similarly for test set and output the accuracy 
k = maxK 
...
numWrong = 0.0 # initialize number of faces wrongly predicted     
j = 0    
for eachTestIm in testIm: 
    # Get the coefficients of reconstruction for this validation image 
    coeffTest = get_coefficients(eigenFaces[:k], eachTestIm, averageFace)
    currMin = sys.float_info.max # initialize to largest possible flaot 
    indexTrain = 0 # initialize index to first training image 
    ...
    for eachTrainIm in trainIm:
        ...
        # Calculate the SSD
        # get the coefficient from train as calculated 
        differenceSSD = np.array(coeffTest) - coeffTrains[i] 
        differenceSSD = differenceSSD * differenceSSD 
        sumSSD = differenceSSD.sum() 
        if sumSSD < currMin:
            currMin = sumSSD
    ...
    # Group into boys and girls 
    # 1 = Boy
    # 2 = Girl 
    A = 1 # Initialize both to boys 
    B = 1
    ...
    if (A == B):
        ...
        numWrong += 1.0
        # Record only the first 5 failure cases 
        if numWrong < 6.0: 
            # Print out the test face 
            imsave("./failureCases/" +  "part4_" + str(int(numWrong)) + "_" +  'testFace.png', eachTestIm.reshape(train1Shape))

            # Print out the training face 
            imsave("./failureCases/" + "part4_" + str(int(numWrong)) + "_" +  'trainFace.png', trainIm[indexTrain].reshape(train1Shape))

            # Print out the eigenTestFace
            imsave("./failureCases/" + "part4_" + str(int(numWrong)) + "_"  +'eigenTestFace.png', get_reconstruction(eigenFace[:k], eachTestIm, averageFace).reshape(train1Shape))

            # Print out the eigenTrainingFace 
            imsave("./failureCases/" + "part4_" + str(int(numWrong)) + "_"   +'eigenTrainFace.png', get_reconstruction(eigenFace[:k], trainIm[indexTrain], averageFace).reshape(train1Shape))
...
#train1Im, train1Shape = get_face_matrix(files_dir, 'eckhart', 0, 10)
#train2Im, train2Shape = get_face_matrix(files_dir, 'sandler', 0, 10)
#train3Im, train3Shape = get_face_matrix(files_dir, 'brody', 0, 10)
#val1Im, val1Shape = get_face_matrix(files_dir, 'eckhart', 10, 15)
#val2Im, val2Shape = get_face_matrix(files_dir, 'sandler', 10, 15)
#val3Im, val3Shape = get_face_matrix(files_dir, 'brody', 10, 15)
#test1Im, test1Shape = get_face_matrix(files_dir, 'eckhart', 15, 20)
#test2Im, test2Shape = get_face_matrix(files_dir, 'sandler', 15, 20)
#test3Im, test3Shape = get_face_matrix(files_dir, 'brody', 15, 20)
