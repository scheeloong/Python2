...
# Note: Must include canny.py as well! 
...
if __name__ == "__main__":
    ...
    for abc in range (2):
        ...
        # binaryCannyPart5 = canny.cannyPart5(imRGBCannyPart5, 4.0, 20, 10)
        im = imRGBCannyPart5
        sigma = 4.0
        thresHigh = 20
        thresLow = 10 
       ...
       # theta[xx, yy] = defaultTheta # set to a value that is known
        ...
        while np.any((canvas == -1).flatten('F')): # Part1 changes: added 
            ...
            delta = np.array([cos(theta[cntr[1]-1, cntr[0]-1] + randomDegree), sin(theta[cntr[1]-1, cntr[0]-1] +  randomDegree)])
            
            # Add each component of the color separately
            
            imRGBPart6 = imRGB.copy() 
            
            colour = np.reshape(imRGB[cntr[1]-1, cntr[0]-1, :],(3,1))
    
            # Generate random color changes for Red Channel 
            
            randomColour = ((np.random.rand(1,1)[0][0] * 30) - 15)/255  #divide the color by 255
            #print randomColour 
            #print colour.shape # (3,1)
            ...
            randomColourScale = (np.random.rand(1,1)[0][0] * 30) - 15   # get between -15 to 15
